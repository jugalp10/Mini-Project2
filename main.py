...
def addAnArticle(dblp):
    clear()
    # create a uique id for the article
    while True:
        uid = str(uuid.uuid4())
        if dblp.find_one({"id": uid}) == None:
            break
    ...
    while True:
        print("Press Y to add author")
        print("Press N to stop adding authors")
        uin = input("Enter choice: ")
        clear()
        if uin == "Y" or uin == "y":
    ...
    dblp.insert_one({"id": uid, "title": title, "author": author, "year": year, "abstract": abstract, "venue": venue, "n_citation": n_citation, "reference": reference})
